...
async function fetchFromAdzuna({ q, location, limit }) {
  ...
  const params = new URLSearchParams({
    ...
    results_per_page: String(Math.min(limit || 12, 50)),
...
async function fetchFromJooble({ q, location, limit }) {
  if (!JOOBLE_API_KEY) {
  ...
  return (data.jobs || []).slice(0, limit || 12).map((j, idx) => ({
