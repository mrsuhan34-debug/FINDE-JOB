...
async function fetchFromAdzuna({ q, location }) {
  ...
  if (!res.ok) throw new Error(`Adzuna responded with ${res.status}`);
...
async function fetchFromJooble({ q, location }) {
  ...
  const res = await fetch(`https://jooble.org/api/${JOOBLE_API_KEY}`, {
    ...
    body: JSON.stringify({ keywords: q || '', location: location || '' }),
  });
  if (!res.ok) throw new Error(`Jooble responded with ${res.status}`);
  const data = await res.json();
